...
void setup_vm_final(void) {
    ...
    create_mapping(swapper_pg_dir,_stext,_stext-PA2VA_OFFSET,2,11);
    printk("mapping kernel text !\n");

    // mapping kernel rodata -|-|R|V
    create_mapping(swapper_pg_dir,_srodata,_srodata-PA2VA_OFFSET,1,3);
    printk("mapping kernel rodata !\n");
    
    // mapping other memory -|W|R|V
    create_mapping(swapper_pg_dir,_sdata,_sdata-PA2VA_OFFSET,32765,7);
...
void create_mapping(uint64 *pgtbl, uint64 va, uint64 pa, uint64 sz, int perm) {
    ...
    */
	...
	2通过pagetable中的VPN段获取PTE。(可以把pagetable看成一个数组，VPN看成下标PAGE SIZE为4KB，
